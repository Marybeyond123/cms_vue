<template>
	<div class="user">
		<!-- 按钮区 -->
		<div class="btns">
			<el-button size='mini' type="success"  @click='toAddUser'>添加</el-button>
		</div>
		<!-- 按钮区结束 -->
		<!-- 数据区 -->
		<div class="user_tbl">
			<ul class="user_list">
				<li v-for='u in users'>
                   ...
                   <div class="content">
					...
					<div class="info"> 
						<div>注册时间</div> <div>{{u.regTime?u.regTime:'--'}}</div> 
					</div>
					<div class="info"> 
						<div>状态</div> <div><el-switch v-model="u.enabled" active-color="#13ce66" inactive-color="#ff4949"></el-switch></div> 
					</div>
                   </div>
				</li>
			</ul>
		...
		<el-dialog :title="userDialog.title" :visible.sync="userDialog.visible">
		  ...
		  <el-form status-icon ref="userForm" :rules='rules' :model="userDialog.form" label-position='left' size='small'>
		    ...
		    <el-form-item label="确认密码" label-width="100px" prop="password1">
		      <el-input v-model="userDialog.form.password1" type="password"></el-input>
		    </el-form-item>
		    ...
		    <el-form-item label="头像" label-width="100px">
		      <el-upload
					  action="http://119.23.209.34:8099/manager/file/upload"
					  :limit='1'
	...
	export default {
		data(){
		        ...
		        if (value === '') {
		          ...
		          if (this.userDialog.form.password1 !== '') {
